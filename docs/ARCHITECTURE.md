...
> Scope v1: piattaforma **web-only**. Il client mobile verrà introdotto in una fase successiva.
...
Il frontend web comunica con **due backend** in base all'operazione:
...
async function authMiddleware(request, reply) {
  ...
  try {
    ...
    return reply
      .code(401)
      .send({ error: "INVALID_TOKEN", detail: err.message });
  }
}
...
Implementazione Angular (Web): sidebar sempre visibile (schermo largo), collassabile.
...
function getTimerDuration(bidCount, settings) {
  ...
  const rule = settings.timer_decay_rules.find(
    (r) => bidCount >= r.from_bid && bidCount <= r.to_bid,
  );
...
function processBid(leagueId, memberId, amount) {
  ...
  if (amount > state.members.get(memberId).budgetCurrent)
...
- **Command Bar:** sfondo scuro, in alto (web)
...
| ------------------------------- | -------- | --------------------------------------------------------------------------------- |
| **Client mobile (iOS/Android)** | Alta     | Sviluppo post-v1, dopo consolidamento esperienza web.                             |
